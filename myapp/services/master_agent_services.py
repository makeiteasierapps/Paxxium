...
class MasterAgentService:
    ...
    def check_and_set_agent_instance(self, uid, chat_id, system_prompt, chat_constants, agent_model, user_analysis=None, conversation_data=None):
        key = (uid, chat_id)
        ...
        if key not in self.master_agents:
            ...
            # remove the oldest one to make space for a new instance
            if len(self.master_agents) >= self.max_agent_instances:
                self.master_agents.popitem(last=False)

            self.master_agents[key] = MasterAgent(self.message_service, uid, chat_id, model=agent_model, system_prompt=system_prompt, chat_constants=chat_constants, user_analysis=user_analysis)
            self.master_agents[key].load_history_to_memory(conversation_data)
        else:
            self.master_agents[key].user_analysis = user_analysis
            self.master_agents[key].chat_constants = chat_constants
            self.master_agents[key].update_llm_instance(agent_model, system_prompt)
