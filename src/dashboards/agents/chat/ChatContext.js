    ...
    try {
      ...
      setAgentArray((prevAgents) => {
        ...
        if (updatedAgentIndex !== -1) {
          localStorage.setItem("agentArray", JSON.stringify(updatedAgents));
        }
    ...
    try {
      ...
      setAgentArray((prevChatArray) => {
        const updatedChatArray = prevChatArray.map((chatObj) =>
          chatObj.chatId === chatId ? { ...chatObj, is_open: false } : chatObj
        );

        // Store the updated array in local storage
        localStorage.setItem("agentArray", JSON.stringify(updatedChatArray));
  ...
  const createChat = async (
    ...
    try {
      ...
      setAgentArray((prevAgents) => {
        ...
        localStorage.setItem("agentArray", JSON.stringify(updatedAgentArray));
