...
class BossAgent:
    ...
    def pass_to_agent_scrape(self, query,content):
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful assistant that takes in the content of a scraped website along with a query from the user. You will return a json object with the following structure: {'result': 'Result of the query', 'summary': 'summary of the content'}"
                },
                {
                    "role": "user",
                    "content": f"Query: {query} Content: {content}",
                }
            ],
        )
